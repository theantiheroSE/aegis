...
A Node.js backup tool for this VPS, with an interactive terminal UI and a first-time setup wizard.

Snapshots every:

- **PM2** project (full directory, excluding `node_modules`, build/cache dirs)
- **PostgreSQL** database (`pg_dump -Fc -Z9` per DB)
- **SQLite** database found under configured search paths (`sqlite3 .backup` for safe online backup)
- **nginx** config tree (`sites-available`, `sites-enabled`, `conf.d`, `nginx.conf`)
- **extra paths** you specify (e.g. `/etc/letsencrypt`, `/root/config`)

Bundled into a single dated `.tar.zst` archive with a `.sha256` sidecar, uploaded to a remote server over **SSH** (recommended) or **FTP/FTPS**, then old remote backups are pruned by age.

One npm dep (`neo-blessed` for the TUI). System tools needed: `node`, `tar`, `zstd`, `pg_dump`, `psql`, `sqlite3`, `pm2`, `rsync`, `ssh`, `runuser`, `curl` (for FTP).
...
After setup, `node backup.mjs` (no flags, on a TTY) launches the dashboard:
...
│ │   Quit            │ │                                                           │ │
│ └───────────────────┘ └────────────────────────────────────────────────────────┘ │
│ q:quit  ↑/↓:navigate  Enter:select  r:refresh  ?:help                             │
└──────────────────────────────────────────────────────────────────────────────────┘
```

During a backup, the right panel switches to a progress view with phase bars and live log:
...
The wizard is just a convenience that asks the same questions and writes the same file.
...
Or use the TUI: pick "Install / update cron" → choose a schedule (or "Custom" to type one).
...
The restore script dispatches on `config.transfer` — SSH uses rsync+ssh, FTP uses curl.
