...
export async function sendNotifications(cfg, { ok, summary }) {
  ...
  const subject = ok
    ? `[Aegis] OK — ${summary.items} items, ${summary.duration}`
    : `[Aegis] FAILED — ${summary.error || "unknown error"}`;
...
// Ping a healthchecks.io / Uptime Kuma style dead-man's switch.
// Independent of notifications.onSuccess / onFailure — a missing backup
// is the more dangerous failure mode and should alert even when the user
// has muted other channels.
//   - Success:  GET  <url>
//   - Failure:  POST <url>/fail
// Both services use the same protocol (a successful GET counts as "alive").
export async function pingHealthcheck(url, ok) {
  if (!url) return;
  const target = ok ? url : (url.endsWith("/") ? url + "fail" : url + "/fail");
  const method = ok ? "GET" : "POST";
...
async function sendWebhook(cfg, payload) {
  ...
  // If a signing secret is set, sign the body with HMAC-SHA256 and add
  // an X-Aegis-Signature header. This is the standard pattern (used by
  // GitHub, Stripe, etc.) so the receiver can verify the call really
  // came from this Aegis instance and wasn't tampered with in transit.
  // Header format matches GitHub's convention: `sha256=<hex>`.
  if (cfg.secret) {
