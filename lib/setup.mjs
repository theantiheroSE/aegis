...
//   - choosing SSH (recommended), FTP, or rclone (S3/B2/R2/etc.)
//   - for SSH: generating a dedicated keypair, displaying the public key
//   - for FTP: collecting host/port/user/password/dir
//   - for rclone: collecting remote name and path (user runs `rclone config`)
...
export function isFirstRun(cfgPath) {
  ...
  try {
    ...
    const hasRclone = !!raw.rclone?.remote;
    ...
    if (transfer === "rclone" && !hasRclone) return { firstRun: true, reason: "rclone block incomplete" };
...
async function setupSsh() {
  ...
  const defaultKeyPath = "/root/.ssh/aegis_ed25519";
...
async function checkRclone() {
  const r = await run("rclone", ["--version"], { stdio: "ignore" });
  return r.code === 0;
}

async function setupRclone() {
  step("rclone transfer — works with S3, Backblaze B2, Cloudflare R2, Google Cloud Storage, etc.");

  if (!await checkRclone()) {
    info("✗ rclone is not installed on this host.");
    info("  Install with: curl https://rclone.org/install.sh | sudo bash");
    info("  Or use your package manager (apt install rclone).");
    throw new Error("rclone not installed");
  }

  info("Before continuing, run `rclone config` to create a named remote.");
  info("Tip: pick S3 provider, fill in access key + secret + region + bucket.");
  const remote = await prompt("rclone remote name (as configured in `rclone config`)");
  if (!remote) throw new Error("remote name is required");
  const path = await prompt("Path inside the remote (e.g. backups/vps)", "backups/vps");

  if (await promptYesNo("Test the rclone connection now?", true)) {
    info("Testing (rclone lsf on the remote)...");
    const r = await run("rclone", ["lsf", `${remote}:${path}`], { stdio: "ignore" });
    if (r.code === 0) {
      info("✓ Connection works.");
    } else {
      info(`✗ Connection failed (exit ${r.code}).`);
      info("  Make sure you ran `rclone config` and that the remote is reachable.");
      if (!(await promptYesNo("Continue anyway?", true))) {
        throw new Error("setup aborted by user");
      }
    }
...
// Runs the host-discovery probes, shows what's found, and applies the user's
// selections to the in-memory config. Mutates `cfg` in place.
async function runScanStep(cfg) {
  const { runScan, applyFindings } = await import("./scan.mjs");
  step("System scan — discovering what's installed on this host…");
  const t0 = Date.now();
  ...
  if (detected.length === 0) {
    info("(no services detected that Aegis knows how to back up)");
    return;
  }

  box(
    `Found ${detected.length} item(s) in ${(ms / 1000).toFixed(1)}s`,
    detected.map((f) => `  • ${f.title}\n      ${f.details}`).join("\n"),
  );

  // Default selection: everything `recommend: true`.
  const recommended = new Set(detected.filter((f) => f.recommend).map((f) => f.id));
  if (recommended.size === 0) {
    info("Nothing to recommend — skipping.");
    return;
  }

  if (await promptYesNo(`Apply all ${recommended.size} recommendation(s)?`, true)) {
    applyFindings(cfg, findings, recommended);
    info(`Applied ${recommended.size} suggestion(s) to config.`);
    return;
  }

  // Per-item picker.
  info("Pick which to apply (comma-separated numbers, 'all', or 'none'):");
  detected.forEach((f, i) => {
    const marker = f.recommend ? "[ ]" : "[i]"; // [i] = informational, no apply fn
    info(`  ${(i + 1).toString().padStart(2)}. ${marker} ${f.title} — ${f.details}`);
  });
  const ans = (await prompt("Apply which? (e.g. 1,3,5 / all / none)", "all")).trim().toLowerCase();
  let selected = new Set();
  if (ans === "all" || ans === "") {
    for (const f of detected) if (f.apply) selected.add(f.id);
  } else if (ans === "none" || ans === "n") {
    // nothing
  } else {
    const nums = ans.split(/[,\s]+/).map((s) => parseInt(s, 10)).filter((n) => Number.isFinite(n));
    for (const n of nums) {
      const f = detected[n - 1];
      if (f && f.apply) selected.add(f.id);
    }
  }
  applyFindings(cfg, findings, selected);
  info(`Applied ${selected.size} suggestion(s) to config.`);
}
...
function buildConfigFromAnswers(answers) {
  ...
  if (answers.transfer === "ssh") {
    ...
    delete cfg.ftp;
    delete cfg.rclone;
  } else if (answers.transfer === "rclone") {
    cfg.rclone = { ...(cfg.rclone || {}), ...answers.rclone };
    cfg.ssh = { identityFile: "/root/.ssh/aegis_ed25519" };
    delete cfg.ftp;
  } else {
...
export async function runSetupWizard(cfgPath, { force = false, nonInteractive = false } = {}) {
  if (!existsSync(cfgPath) || force) {
    ...
    header(`Aegis setup (${status.reason})`);
    info(`Config path: ${cfgPath}\n`);
  }
  ...
  const choice = await promptChoice(
    ...
    [
      ...
      { label: "rclone  (S3, B2, R2, ...)", description: "any rclone-supported object store" },
  ...
  else if (choice === 2) answers = await setupRclone();
  else throw new Error("setup cancelled");

  const cfg = buildConfigFromAnswers(answers);

  // System scan — discover what's installed and pre-fill backup targets.
  // Runs after the connection test, before writing config.
  if (await promptYesNo("Scan the system to discover what to back up?", true)) {
    await runScanStep(cfg);
  }
  ...
  box(
    ...
    (cfg.transfer === "ssh"
      ...
      : cfg.transfer === "rclone"
      ? `rclone remote:     ${cfg.rclone.remote}:${cfg.rclone.path}`
